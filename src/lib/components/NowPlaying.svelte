<script lang="ts">
	...
	import Tracks from './Track/Track.svelte';
...
<section class="p-4">
	<div class="h-20 content-center">
		<div class="gap-2 rounded-lg border-2 bg-gray-100 p-4 text-sm text-gray-500">
			{#if song.title}
				<span class="leading-0"
					><span>Jetzt läuft: </span><span class="ml-4 font-semibold">{song.title}</span>
					- {song.artist}</span
				>
			{:else}
				<svg
